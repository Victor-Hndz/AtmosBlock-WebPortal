...
int main(int argc, char **argv) {
    ...
    for (time=0; time<NTIME; time++) {
        ...
        t_ini = omp_get_wtime();
        #pragma omp parallel num_threads(N_THREADS) shared(z, lats, lons, size_x, size_y, time, selected_points, filtered_points, step, scale_factor, offset, PARAMS, FILA_LAT_INICIO, COL_LON_INICIO) default(none)
        {
            int lat, lon;
