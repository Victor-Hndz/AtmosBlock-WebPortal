...
| 2. Zona | Se elige qué parte del mundo se mira; hacia el polo se llega hasta el final. | Dominio en latitud entre el límite hacia el ecuador y ±90°: el límite polar no recorta el análisis. En longitud recorta el fichero de entrada, no el programa. |
...
| 4b. Un matiz | Un rayo que se sale de la zona mirada cuenta como “más bajo”. | Los rayos que caen fuera del dominio suman al recuento de MAX, no al de MIN: asimetría conocida que solo afecta a los bordes de dominios regionales, anotada como deuda. |
...
| 6. Descartar | Fuera lo demasiado pequeño y lo demasiado tropical. | Área del cluster (suma de celdas R²·Δλ·Δφ·cos φ) ≥ 22 000 km² y punto más cercano al polo por encima de 30°. Sin límite polar por defecto. |
...
| 8. Resultado | Una tabla con lo detectado y la receta exacta que se usó. | CSV de puntos y de formaciones, con toda la configuración en la cabecera; figuras en Python. |
...
| | En llano | En técnico |
|---|---|---|
| **Omega (Ω)** | Una zona de aire alto con una zona baja a cada lado; el aire la rodea dibujando una omega y la circulación se queda parada. | Alta cerrada al este y al oeste y abierta hacia el ecuador, con dos mínimos flanqueantes, uno a cada lado en longitud, no más cerca del polo que ella y a menos de 3000 km (`search_radius_km`). |
